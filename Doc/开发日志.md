...
## 18. SubArgument 创建工作流 + 持久化修复 (2026-02-21)

### 18.1 功能概述

实现完整的 SubArgument 手动创建工作流：
1. 用户点击 "+" 创建空白 SubArgument
2. 输入标题后自动触发 AI 推荐 Snippets
3. 推荐的 Snippets 以 `pendingSnippetIds` 存储，显示红点指示器
4. 用户进入 "Edit Snippets" 模式确认/修改
5. 确认后转为正式 `snippetIds`

### 18.2 新增字段

**SubArgument 类型扩展** (`types/index.ts`):
```typescript
interface SubArgument {
  // ...existing fields
  pendingSnippetIds?: string[];      // AI 推荐待确认
  needsSnippetConfirmation?: boolean; // 是否需要用户确认
}
```

### 18.3 Bug 修复

#### 18.3.1 Evidence Cards 不显示推荐 Snippets

**问题**: 新建 SubArgument 的 `pendingSnippetIds` 不在 Evidence Cards 中显示

**原因**: `EvidenceCardPool.tsx` 中 `isFilteredOutBySubArgument` 只检查 `snippetIds`，未检查 `pendingSnippetIds`
...
const isFilteredOutBySubArgument = useMemo(() => {
  ...
  return !inSnippetIds && !inPendingIds;  // 检查两个列表
}, [focusState, subArguments, snippet.id]);
```

#### 18.3.2 标题不持久化

**问题**: 刷新页面后 SubArgument 标题丢失

**原因**: `handleSubArgumentTitleChange` 只更新 React 状态，未调用后端 API

**修复**: 添加 `PUT /arguments/{project_id}/subarguments/{id}` 调用

#### 18.3.3 删除不持久化

**问题**: 删除 SubArgument 后刷新页面又出现

**原因**: 
1. 前端加载逻辑 `if (sub_arguments.length > 0)` 阻止空数组覆盖 localStorage
2. 后端 DELETE API 返回 404

**修复 1**: 移除 `length > 0` 条件，始终调用 `setSubArguments`

**修复 2**: FastAPI 路由顺序问题（见 18.4）

### 18.4 FastAPI 路由顺序问题

**问题**: `DELETE /arguments/yaruo_qu/subarguments/subarg-xxx` 返回 404

**根本原因**: FastAPI 按注册顺序匹配路由
- 第 274 行: `@router.delete("/{project_id}/{argument_id}")` (先注册)
- 第 697 行: `@router.delete("/{project_id}/subarguments/{subargument_id}")` (后注册)

当请求 `/subarguments/subarg-xxx` 时，先匹配通用路由，`argument_id = "subarguments"`，找不到返回 404。

**修复**: 在 `delete_argument` 函数中添加前置检查，检测到 `subarg-` 前缀时内联执行删除逻辑：
```python
...
**新增/修改端点** (`backend/app/routers/arguments.py`):

| 方法 | 路径 | 功能 |
|------|------|------|
| POST | `/{project_id}/subarguments` | 创建 SubArgument |
| PUT | `/{project_id}/subarguments/{id}` | 更新 SubArgument (title, snippets, pending_snippet_ids) |
| DELETE | `/{project_id}/subarguments/{id}` | 删除 SubArgument |

**UpdateSubArgumentRequest 模型**:
```python
...
**文件**: `backend/app/services/snippet_recommender.py`

功能:
- `recommend_snippets_for_subargument()`: 根据标题推荐相关 Snippets (LLM 精排)
- `create_subargument()`: 创建并持久化 SubArgument
- `infer_relationship()`: 根据子论点标题推断与父论点关系

### 18.7 修改文件清单

| 文件 | 修改内容 |
|------|----------|
| `frontend/src/types/index.ts` | 添加 `pendingSnippetIds`, `needsSnippetConfirmation` 字段 |
| `frontend/src/context/AppContext.tsx` | 修复加载逻辑、添加 DELETE API 调用 |
| `frontend/src/components/EvidenceCardPool.tsx` | 修复 snippet 过滤逻辑 |
| `frontend/src/components/ArgumentGraph.tsx` | 添加标题持久化、删除按钮 |
| `backend/app/routers/arguments.py` | 添加 PUT/DELETE SubArgument 端点、路由顺序修复 |
| `backend/app/services/snippet_recommender.py` | 新建，推荐 snippets 服务 |

---

## 19. EB-1A 写作策略深度扩展 + P0/P1 修复
...
对比 AI 生成结果与律师例文（Dehuan Liu + Yaruo Qu），发现核心差距在 **prompt 深度**：

| 维度 | 律师例文 | AI 输出 (修复前) |
|------|---------|-----------------|
| 每条 standard 段落数 | 8-15 句 | 3-6 句 |
| 人名引用 | 带头衔全名 + 2-3 句 bio | 泛指 "an expert", "a researcher" |
| 章程引用 | 逐字引用 Article 条款 | 概述性描述 |
| 接受率计算 | COMPUTE 15/648=2.3% | 无 |
| 共同获奖人 | 3-5 人完整介绍 | 提及但无 bio |

### 19.2 修改内容

#### A. Appendix 扩展 (writing_strategies.py)

将 `_EB1A_APPENDICES` 8 个条目从 ~400 字符骨架扩展到 ~1500-2300 字符的结构化写作指南：

```
STRUCTURE: 每个 [award/association/publication/...] 单独一节
PARAGRAPH 1 — 实体背景 (成立年份、治理、声望标志)
PARAGRAPH 2 — 选拔标准/入会流程 (逐字引用章程条款)
PARAGRAPH 3 — 定量数据 (COMPUTE 接受率、排名)
PARAGRAPH 4 — 同行对比 (共同获奖人/成员的资历)
CLOSING: 回扣法规标准
```

#### B. 句子范围更新 (_EB1A_STRATEGIES)

| Standard | 旧 sentence_range | 新 sentence_range | 依据 |
|----------|-------------------|-------------------|------|
| awards | (3,6) | (6,12) | 律师例文每个奖项 10-15 句 |
| membership | (3,5) | (6,12) | 章程引用 + 流程 + 共同成员 |
| published | (3,5) | (6,10) | 出版物背景 + 发行量 + 范围 |
| judging | (3,5) | (6,12) | 角色 + 规模 + 共同评委 + 影响 |
| contribution | (4,7) | (8,15) | 最密集：技术 + 商业化 + 受益者 + 专家 |
| scholarly | (3,5) | (6,12) | 期刊链 + 百分位 + 跨学科 + 推荐信 |
| leading | (4,6) | (8,15) | 最长：机构 + 角色 + 量化影响 + 下游成果 |
| salary | (3,5) | (5,10) | 金额 + 基准 + 倍数 + 补充收入 |

#### C. max_tokens 对齐 (petition_writer_v3.py)

| Step | 修改前 | 修改后 | 原因 |
|------|--------|--------|------|
| Step 2a (polish_single) | 4096 | 8000 | 扩展后句子范围需要更大输出空间 |
| Step 2b (polish_multi) | 8192 | 8000 | 对齐到 DeepSeek API 实际最大值 |

### 19.3 P0/P1 修复

**P0 — 人名引用规则**:
- 在 `_eb1a_instruction()` (用户 prompt) 中添加 PERSON NAMING 强制规则
- 要求使用带头衔全名 + 2-3 句 bio，禁止泛指
- **关键发现**: DeepSeek 对 system prompt 附录中的细节遵从度很低，但对 user prompt instruction block 遵从度高

**P1 — 逐字引用规则**:
- 在 `_eb1a_instruction()` 中添加 VERBATIM QUOTING 强制规则
- 要求章程、章程条款、推荐信评价必须用引号原文引用

**效果** (以 leading_role 为例):
| 指标 | 修复前 | 修复后 |
|------|--------|--------|
| 带头衔人名 | 1 | 7 |
| 逐字引用 | 0 | 19 |
...
15 个 standard 全部通过 (dehuan_liu 9 + yaruo_qu 6)：

| 项目 | Standard | 句子数 | 词数 | 带头衔人名 | 逐字引用 | 耗时 |
|------|----------|--------|------|-----------|---------|------|
| dehuan_liu | awards | 21 | 732 | 0* | 7 | 93s |
| dehuan_liu | judging | 22 | 806 | 5 | 12 | 102s |
| dehuan_liu | scholarly | 26 | 902 | 4 | 8 | 109s |
| dehuan_liu | leading | 48 | 1812 | 7 | 19 | 291s |
| dehuan_liu | salary | 15 | 574 | 2 | 5 | 67s |
| yaruo_qu | membership | 28 | 1082 | 6 | 9 | 131s |
| yaruo_qu | published | 12 | 434 | 3 | 4 | 58s |
| yaruo_qu | contribution | 29 | 1128 | 5 | 11 | 147s |
| yaruo_qu | leading | 37 | 1483 | 8 | 15 | 204s |

*awards 人名为 0 是因为 Exhibit B6/B7 (人物介绍) 不在该 argument 的 snippet_ids 中，属于上游 argument assembly 的缺口。

### 19.5 修改文件清单

| 文件 | 修改内容 |
|------|----------|
| `backend/app/services/writing_strategies.py` | 新建；8 条 EB-1A appendix 扩展 + sentence_range + P0/P1 规则 |
| `backend/app/services/petition_writer_v3.py` | max_tokens 4096→8000, 8192→8000 |

---

## 20. Snippet 页码归因 Bug 修复
...
NIW 写作输出中大量引用 `[Exhibit X, p.1]`，但所有 exhibit 的第 1 页都是封面（仅含 "Exhibit C-7" 等文字）。

### 20.2 根因分析

**Bug 位置**: `snippet_extractor.py` → `format_blocks_for_llm()`

```python
# 文档 JSON 中 block_id 已含页码前缀: p2_b0
block_id = block.get("block_id", "")  # → "p2_b0"

# 旧代码又加了一层前缀
composite_id = f"p{page_num}_{block_id}"  # → "p2_p2_b0" (冗余!)
```

**影响链路**:

```
snippet_extractor: composite_id 冗余前缀 "p2_p2_b0"
    ↓ LLM 提取时归因错误，所有 snippet → page=1
snippet_registry: page=1 (错误)
    ↓
load_exhibit_pages_for_argument: 只加载 page 1+2 (丢失 page 3~N)
    ↓
写作 LLM: 看到 snippet 全标 p.1，自然引用 "[Exhibit X, p.1]"
    ↓
_backfill_snippet_ids: (exhibit_id, 1) → 匹配所有 snippet (数据本身错误)
```

**影响范围**:

| 项目 | 总 exhibit | 受影响 exhibit (100% page=1) | 受影响 snippet |
|------|-----------|---------------------------|--------------|
| chen_zhen | 38 | 15 (39%) | 157 |
| xiaoyang_wang | 38 | 14 (37%) | 170 |
| dehuan_liu | - | 0 | 0 |
| yaruo_qu | - | 0 | 0 |

### 20.3 修复方案

#### A. 代码修复 (snippet_extractor.py)

```python
# 修复后: 检测 block_id 是否已含页码前缀
if re.match(r'^p\d+_', block_id):
    composite_id = block_id          # p2_b0 → 直接使用
else:
    composite_id = f"p{page_num}_{block_id}"  # 兼容无前缀的文档
```

#### B. 数据修复 (fix_snippet_pages.py)

对已有数据做后处理修复：
1. 识别 suspect exhibit (所有 snippet page=1 且 page 1 为封面)
2. 对每个 snippet，用 `SequenceMatcher` 在文档所有页的 block 中匹配最佳文本
3. 匹配分数 ≥ 0.3 时更新 page 和 block_id

### 20.4 验证结果

**数据层面**:

| 项目 | 修复前 page=1 比例 | 修复后 | 修复 snippet 数 |
|------|-------------------|--------|----------------|
| chen_zhen | 38.5% | 0.2% | 157 |
| xiaoyang_wang | ~37% | 1.1% | 170 |

**写作输出层面** (6 个 NIW standard 全量重跑):

| 项目 | Standard | 修复前 p.1 引用 | 修复后 p.1 引用 | 变化 |
|------|----------|----------------|----------------|------|
| chen_zhen | prong1_merit | 9 | 0 | -9 |
| chen_zhen | prong2_positioned | 9 | 0 | -9 |
| chen_zhen | prong3_balance | 0 | 0 | 0 |
| xiaoyang_wang | prong1_merit | 12 | 1 | -11 |
| xiaoyang_wang | prong2_positioned | 36 | 1 | -35 |
| xiaoyang_wang | prong3_balance | 0 | 0 | 0 |
| **TOTAL** | | **66** | **2** | **-64 (-97%)** |

剩余 2 个 p.1 引用为合理引用（对应 exhibit 确有少量 page 1 内容）。

### 20.5 修改文件清单

| 文件 | 修改内容 |
|------|----------|
| `backend/app/services/snippet_extractor.py` | composite_id 去重复前缀 + 兼容逻辑 |
| `backend/fix_snippet_pages.py` | 新建；后处理修复脚本 |
| `backend/data/projects/chen_zhen/extraction/combined_extraction.json` | 157 个 snippet page 修正 |
| `backend/data/projects/xiaoyang_wang/extraction/combined_extraction.json` | 170 个 snippet page 修正 |
