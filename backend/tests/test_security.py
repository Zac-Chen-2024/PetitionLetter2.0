...
def test_traversal_ids_rejected_on_delete(client, tmp_data_dir, victim_project, bad_id):
    ...
    assert (tmp_data_dir / "projects" / "victim").exists()


def test_dotdot_project_id_does_not_wipe_data_dir(client, tmp_data_dir, victim_project):
    ...
    assert r.status_code == 404
    assert sorted(p.name for p in tmp_data_dir.iterdir()) == ["projects", "style_templates"]
...
def test_storage_get_project_dir_rejects_unsafe_ids(tmp_data_dir):
    ...
    assert ok.parent == tmp_data_dir / "projects"


def test_500_does_not_leak_exception_text(client, tmp_data_dir, victim_project):
    # corrupt meta.json -> json.JSONDecodeError inside the handler
    (tmp_data_dir / "projects" / "victim" / "meta.json").write_text("{not json", encoding="utf-8")
